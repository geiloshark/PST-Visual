      ...
      fluidRow(
        ...
        column(8,
          wellPanel(
            ...
            br(),
            h4("Life-history samples and estimated Reference Points"),
            div(style = "overflow-x: auto;",
              tableOutput("om_combined_tbl")
            )
          )
        )
      )
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(
    ...
    om_combined_df = NULL,  # merged values + targets table after initialise
  ...
  observeEvent(input$run_om, {
    ...
    result <- tryCatch({
      ...
      if (!is.null(input$om_upload)) {
        ...
        combined_df <- merge(val_df, tgt_df, by = "sample")
        ...
        list(ok = TRUE, obj = obj, combined_df = combined_df,
    ...
    rv$om_ok        <- result$ok
    rv$om_combined_df <- result$combined_df  # NULL on error path
  ...
  output$om_combined_tbl <- renderTable({
    rv$om_combined_df
